...
namespace pack
{
	namespace detail
	{
		...
		struct source : ISeqInStream
		{
			...
			static SRes read(void* p, void* buf, size_t* size)
			{
                ...
                size_t n = std::min(2048ul, std::min(*size, self->cb));
	...
	bool lzma_unpack(void const* src, size_t cb, Iterator I)
	{
		...
		cb -= 5;
        SizeT _cb_i = cb_i;
        SRes res = Lzma2Dec_DecodeToBuf(&dec.dec, &buf[0], &_cb_i, (Byte const*)src + 5, &cb, LZMA_FINISH_END, &status);
		if (res == SZ_OK && status == LZMA_STATUS_FINISHED_WITH_MARK)
		{
            os.Write(&os, &buf[0], _cb_i);
			return true;
		}
		return false;
	}
